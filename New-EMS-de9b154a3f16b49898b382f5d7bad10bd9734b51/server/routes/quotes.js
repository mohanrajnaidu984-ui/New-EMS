...
// Helper: get latest Base Price for a specific Request + Job + Customer
async function getLatestBasePrice({ requestNo, jobName, customerName }) {
    const result = await sql.query`
        ...
        JOIN EnquiryPricingValues  v
            ON v.RequestNo = o.RequestNo
           AND v.OptionID  = o.ID
        WHERE o.RequestNo    = ${requestNo}
          AND o.ItemName     = ${jobName}
          AND o.OptionName   = 'Base Price'
          AND v.CustomerName = ${customerName}
        ORDER BY v.UpdatedAt DESC, v.ID DESC;
    `;

    if (!result.recordset.length) {
        return { basePrice: 0, updatedAt: null };
    }

    return {
        basePrice: Number(result.recordset[0].Price) || 0,
            ...
            const mappedEnquiries = enquiries.map(enq => {
                ...
                const subJobPrices = filteredFlatList.map(job => {
                    ...
                    const displayName = (() => {
                        ...
                        // Always show "Name (L1/L2/...)" for both Lead and Sub jobs in pending quote summaries
                        return `${job.ItemName} (${displayCode})`;
                    })();
...
// GET /api/quotes/pricing-summary?requestNo=25&jobName=BMS%20Project&customerName=HVAC%20Project
router.get('/pricing-summary', async (req, res) => {
    try {
        const { requestNo, jobName, customerName } = req.query;
        if (!requestNo || !jobName || !customerName) {
            return res.status(400).json({ error: 'Missing requestNo, jobName or customerName' });
        }

        const summary = await getLatestBasePrice({
        ...
        console.error('Error in /api/quotes/pricing-summary', err);
        res.status(500).json({ error: 'Failed to load pricing summary' });
    }
});
...
router.get('/enquiry-data/:requestNo', async (req, res) => {
    try {
        ...
        try {
            ...
            // Build root index map for this enquiry (Enquiry Data Fetch)
            const roots = rawItems.filter(j => !j.ParentID || j.ParentID == '0' || j.ParentID == 0);
            roots.sort((a, b) => a.ID - b.ID);
            const rootLabelMap = {};
            roots.forEach((r, idx) => {
                const existing = (r.LeadJobCode || '').trim().toUpperCase();
            ...
            divisionsHierarchy = rawItems.map(r => {
                let curr = r;
                let depth = 0;
                while (curr && !rootLabelMap[curr.ID] && curr.ParentID && curr.ParentID != '0' && depth < 20) {
                    const p = rawItems.find(i => i.ID === curr.ParentID);
                    if (!p) break;
                    curr = p;
                    depth++;
                }
                const assignedCode = (curr && rootLabelMap[curr.ID]) ? rootLabelMap[curr.ID] : 'L1';
                ...
                return {
                    ...
                    leadJobCode: assignedCode, // Use assigned L-code
