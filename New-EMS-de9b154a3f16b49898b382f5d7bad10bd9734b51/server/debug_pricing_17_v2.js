...
async function run() {
    try {
        ...
        const enq = await request.query(`SELECT RequestNo, ProjectName, CreatedBy, CustomerName, ClientName, ConsultantName FROM EnquiryMaster WHERE RequestNo = '17' OR ID = 17`);
        console.table(enq.recordset);
        if (enq.recordset.length === 0) {
            console.log('No Enquiry found with ID 17');
            process.exit(1);
        }
