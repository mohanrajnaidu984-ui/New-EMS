...
for (let i = 0; i < content.length; i++) {
    ...
    if (char === '{') {
        ...
        if (stack.length === 0) {
            if (line_num > 204 && line_num < 5851) {
               console.log(`SUSPICIOUS: Stack became empty at line ${line_num}, col ${col_num}. Last opened '{' was from line ${popped.line}, col ${popped.col}`);
               printContext(content, line_num);
            }
        }
    }
