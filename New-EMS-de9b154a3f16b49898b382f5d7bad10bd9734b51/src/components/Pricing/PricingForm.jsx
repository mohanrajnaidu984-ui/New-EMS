...
const PricingForm = () => {
    ...
    const saveAll = async () => {
        ...
        for (const key of allKeys) {
            if (key.startsWith('simulated')) continue;
            ...
            if (!editableJobs.includes(job.itemName)) continue;
    ...
    const filteredOptions = React.useMemo(() => {
        ...
        // Step 1: Broad Filter (Context + Customer/Internal Match)
        const candidates = pricingData.options.filter(o => {
            if (activeLeadName) {
                if (o.leadJobName) {
                    if (o.leadJobName !== activeLeadName) return false;
                } else if (o.itemName && !leadScope.has(o.itemName)) {
                    return false;
                }
            }
            ...
            if (isCustomerMatch) return true;

            // INTERNAL FALLTHROUGH: Allow sub-job options quoting to parent (Natural customer)
            if (o.itemName && o.customerName) {
                const job = pricingData.jobs.find(j => j.itemName === o.itemName);
                if (job && job.parentId) {
                    const parent = pricingData.jobs.find(p => p.id === job.parentId);
                    if (parent) {
                        const cleanP = parent.itemName.replace(/^(L\d+|Sub Job)\s*-\s*/i, '').trim();
                        return o.customerName === parent.itemName || o.customerName === cleanP;
                    }
                }
            }
            return false;
        ...
        candidates.forEach(o => {
            const dedupKey = `${o.name}-${o.itemName}-${o.leadJobName || 'Legacy'}`;
                            ...
                            return (
                                <div style={{ padding: '12px 20px', background: '#f1f5f9', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    ...
                                    >
                                        ...
                                        {roots.map(r => {
                                             const displayLabel = r.leadJobCode || r.LeadJobCode || r.itemName;
                                             return <option key={r.id} value={r.id}>{displayLabel}</option>;
                                         })}
                                    </select>
                                </div>
                            );
                                    ...
                                    <table style={{ width: 'auto', minWidth: '600px', borderCollapse: 'collapse' }}>
                                        <tbody>
                                            {(() => {
                                                ...
                                                if (job.isLead) {
                                                    const leadCode = job.leadJobCode || job.LeadJobCode || 'L1';
                                                    // Display as "Landscape Maint (L1)" etc.
                                                    groupName = `${job.itemName} (${leadCode})`;
                                                }
