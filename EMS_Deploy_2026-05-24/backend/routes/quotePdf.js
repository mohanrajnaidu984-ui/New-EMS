...
router.post('/generate', express.json({ limit: '50mb' }), async (req, res) => {
    ...
    try {
        ...
        // Only execute Puppeteer generation when launch probe succeeds to prevent hanging and resource leaks
        const probe = await runLaunchProbe(puppeteer, chromeExe);
        if (!probe.ok) {
            console.warn('[quote-pdf] Launch probe failed during /generate request:', probe.error);
            return res.status(503).json({
                error: 'launch_probe_failed',
                message: 'Use browser Save as PDF',
                hint: 'Chrome did not start or load the quote in time (Windows service isolation). Please generate the PDF via Download PDF (Save as PDF) on the quote tab.',
