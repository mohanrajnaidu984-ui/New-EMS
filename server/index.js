...
app.get('/api/generate-request-no', async (req, res) => {
    try {
        const now = new Date();
        const year = now.getFullYear();
        const month = String(now.getMonth() + 1).padStart(2, '0');
        const prefix = `EYS/${year}/${month}/`;

        let isUnique = false;
        let generatedReqNo = '';
        let counter = 1;

        // Find the first available sequential number starting from 01
        while (!isUnique && counter < 10000) {
            const suffix = String(counter).padStart(2, '0');
            generatedReqNo = `${prefix}${suffix}`;

            const result = await sql.query`SELECT RequestNo FROM EnquiryMaster WHERE RequestNo = ${generatedReqNo}`;
            if (result.recordset.length === 0) {
                isUnique = true;
            } else {
                counter++;
        ...
        if (!isUnique) {
            return res.status(500).send('Failed to generate unique RequestNo');
        }

        res.json({ requestNo: generatedReqNo });
    } catch (err) {
        console.error('Error generating RequestNo:', err);
        res.status(500).send('Server Error');
    }
});
