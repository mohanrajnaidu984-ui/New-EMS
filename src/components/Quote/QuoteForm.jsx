...
const QuoteForm = () => {
            ...
            if (!isLeadJobContext) {
                ...
                if (currentJob) {
                    ...
                    if (parentJob) {
                        ...
                        // Use concise L-tag if parent is the root lead job (Step Lead Job Fix)
                        const isParentRoot = !(parentJob.parentId || parentJob.ParentID) || (parentJob.parentId || parentJob.ParentID) == '0' || (parentJob.parentId || parentJob.ParentID) == 0;
                        const tabDisplay = isParentRoot ? (parentJob.leadJobCode || parentJob.LeadJobCode || 'L1') : pName;
    ...
    }, [enquiryCustomerOptions, toName, activeQuoteTab, enquiryData]);
    ...
    const loadPricingData = async (reqNo, cxName) => {
        ...
        try {
            ...
            if (pricingRes.ok) {
                ...
                const activeLeadKey = normalize(pData.leadJob || '');

                const getBucket = (cKey) => {
                    const custBucket = groupedValues[cKey] || {};
                    return {
                        ...(custBucket['legacy'] || {}),
                        ...(custBucket[activeLeadKey] || {})
                    };
                };

                pData.values = {
                    ...getBucket(mainKey),
                    ...getBucket(currentCustKey)
                };
    ...
    // Calculate Summary based on selected jobs - now delegated to backend API
    const calculateSummary = async (
        data = pricingData,
        currentSelectedJobs = selectedJobs,
        activeCustomer = toName,
        overrideScope = null
    ) => {
        if (!data || !enquiryData) {
            setPricingSummary([]);
        ...
        const requestNo = enquiryData.requestNo || enquiryData.RequestNo;
        let activeTabObj = (calculatedTabs || []).find(t => String(t.id) === String(activeQuoteTab));
        const activeTabRealId = activeTabObj?.realId || activeTabObj?.id;
        const activeJob = (pricingData?.jobs || []).find(
            j => String(j.id || j.ItemID || j.ID) === String(activeTabRealId)
        );
        const jobName = activeJob?.itemName;
        const customer = activeCustomer;

        if (!requestNo || !jobName || !customer) {
            setPricingSummary([]);
            setHasUserPricing(false);
            return;
        }
        ...
        try {
            const url = `${API_BASE}/api/quotes/pricing-summary` +
                `?requestNo=${encodeURIComponent(requestNo)}` +
                `&jobName=${encodeURIComponent(jobName)}` +
                `&customerName=${encodeURIComponent(customer)}`;
            ...
            const base = Number(json.basePrice) || 0;

            if (base > 0) {
                setPricingSummary([{
                    name: jobName,
                    items: [{ name: 'Base Price', total: base }]
                }]);
            ...
            console.error('calculateSummary (backend) failed', e);
            setPricingSummary([]);
            setHasUserPricing(false);
        }

        const activeJobs = currentSelectedJobs || [];
        const summary = [];
        ...
        activeTabObj = (calculatedTabs || []).find(t => String(t.id) === String(activeQuoteTab));
        ...
        const getEffectiveBucket = (cKey) => {
            const custBucket = (data.allValues || {})[cKey] || {};
            return {
                ...(custBucket['legacy'] || {}),
        ...
        const effectiveValuesLookup = (data.allValues)
            ? { ...getEffectiveBucket(mainKey), ...getEffectiveBucket(activeCustKey) }
            : (data.values || {});
        ...
        uniqueOptions.forEach(opt => {
            ...
            if (opt.leadJobName && !isLeadMatch) {
                ...
                if (optLeadId && !branchIds.has(optLeadId)) {
                    ...
                    // NEW: if the option's lead job name matches the active internal customer
                    // (e.g. quoting to "HVAC Project" as internal customer), always allow it.
                    const activeCustNorm = normalizeCust(activeCustomer);
                    const isForActiveInternalCustomer = optLead && optLead === activeCustNorm;

                    if (!isAncestorOfRoot && !isForActiveInternalCustomer) {
            ...
            // 0. Customer Filter (already relaxed for summary – always pass)
            const isCustomerMatch = true;
            ...
            // 1. Visibility Filter (relaxed for quote summary)
            // For the quote preview we want to show the latest prices for the
            // current lead-job branch regardless of fine-grained visibility rules.
            // At this point we've already applied lead-job and branch filters,
            // so we simply treat all remaining options as visible.
            ...
            if (data.jobs) {
                data.jobs.forEach(job => {
                    ...
                    // If we still have no price for this (option, job) pair, but the option
                    // is priced somewhere in the dataset (e.g. per-customer), fall back to
                    // the latest non-zero price for that option ID so the summary is never zero.
                    if (price <= 0) {
                        const allValueBuckets = data.allValues
                            ? Object.values(data.allValues).filter(Boolean)
                            : [data.values || {}];

                        let fallbackPrice = 0;
                        allValueBuckets.forEach(bucket => {
                            Object.values(bucket || {}).forEach(v => {
                                if (String(v.OptionID) === String(opt.id || opt.ID)) {
                                    const p = parsePrice(v.Price || 0);
                                    if (p > 0) fallbackPrice = Math.max(fallbackPrice, p);
                                }
                            });
                        });
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {visibleLeadJobs.map(div => {
                                                    // Find corresponding job object to get its leadJobCode (and ID for ordering)
                                                    const jobObj = enquiryData.divisionsHierarchy?.find(j => j.itemName === div);

                                                    // Determine this root's index among all visible roots to build distinct codes (L1, L2, L3, ...)
                                                    const rootIndex = visibleLeadJobs.findIndex(name => name === div);
                                                    const code = `L${rootIndex + 1}`;

                                                    const label = jobObj?.itemName
                                                        ? `${code} - ${jobObj.itemName}`
                                                        : `${code} - ${div}`;

                                                    return <option key={div} value={div}>{label}</option>;
                                                })}
                                            </select>
                                        );
                                    })()}
