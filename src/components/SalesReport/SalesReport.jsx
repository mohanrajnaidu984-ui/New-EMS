...
const SalesReport = () => {
    ...
    const [isCcMember, setIsCcMember] = useState(false);
    ...
    React.useEffect(() => {
        if (filterLocks.company) return; // locked from user access details
    ...
    React.useEffect(() => {
        if (filterLocks.division) return; // locked from user access details
    ...
    const fetchItemWiseStats = async () => {
        try {
            ...
            const res = await fetch(`/api/sales-report/item-wise-stats?${params.toString()}`);
            if (res.ok) {
                const data = await res.json();
    ...
    const handleChartClick = async (data, metric, extraParam = null) => {
        ...
        try {
            ...
            const res = await fetch(`/api/sales-report/drilldown-details?${params.toString()}`);
            if (res.ok) {
    ...
    const handleFunnelClick = async (data, index) => {
        ...
        try {
            ...
            const res = await fetch(`/api/sales-report/funnel-details?${params.toString()}`);
            if (res.ok) {
                const details = await res.json();
                setFunnelDetails(details);
            }
        ...
        >
            ...
            <div className="d-flex align-items-center justify-content-between mb-2 flex-shrink-0">
                <div className="d-flex gap-2">
                    ...
                    <select className="form-select form-select-sm" style={{ width: 210 }} value={company} onChange={handleCompanyChange} disabled={filterLocks.company}>
                        <option value="All">All Company</option>
                        {filterOptions.companies.map(c => (
                            <option key={c} value={c}>{c}</option>
                        ))}
                    </select>
                    <select className="form-select form-select-sm" style={{ width: 140 }} value={division} onChange={handleDivisionChange} disabled={filterLocks.division}>
                        <option value="All">All Division</option>
                        {filterOptions.divisions.map(d => (
                            <option key={d} value={d}>{d}</option>
                        ))}
                    </select>
                    <select className="form-select form-select-sm" style={{ width: 140 }} value={role} onChange={(e) => setRole(e.target.value)} disabled={filterLocks.role}>
            ...
            <div className="d-flex w-100 flex-grow-1" style={{ minHeight: 0, gap: '0.75rem' }}>
                ...
                <div className="d-flex flex-column" style={{ flex: 1, minWidth: 0, gap: '0.75rem' }}>
                    ...
                    <div className="d-flex w-100" style={{ flex: '57', minHeight: 0, gap: '0.75rem' }}>
                        ...
                        <div className="card shadow-sm border-0 p-2 position-relative pt-2" style={{ flex: '1', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
                            ...
                            {/* Chart Section */}
                            <div className="flex-grow-1" style={{ width: '100%', minHeight: 180, height: 180, flexShrink: 0 }}>
                                <ResponsiveContainer width="100%" height={180} minWidth={100} minHeight={180} debounce={50}>
                        ...
                        <div className="card shadow-sm border-0 p-2 position-relative pt-2" style={{ flex: '1', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div className="flex-grow-1" style={{ width: '100%', minHeight: 180, height: 180, position: 'relative', flexShrink: 0 }}>
                    ...
                    <div className="d-flex w-100" style={{ flex: '43', minHeight: 0, gap: '0.75rem' }}>
                        ...
                        <div className="card shadow-sm border-0 p-2" style={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
                            <div className="d-flex flex-column h-100">
                                ...
                                <div className="flex-grow-1" style={{ width: '100%', minHeight: 150, height: 150, flexShrink: 0 }}>
                                    <ResponsiveContainer width="100%" height={150} minHeight={150} debounce={50}>
                        ...
                        <div className="card shadow-sm border-0 p-2" style={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
                            <div className="position-relative h-100 d-flex flex-column">
                                ...
                                <div className="flex-grow-1" style={{ width: '100%', minHeight: 200, height: 200, flexShrink: 0 }}>
                                    <ResponsiveContainer width="100%" height={200} minHeight={200} debounce={50}>
                                        <PieChart>
                                            <Pie
                ...
                <div className="d-flex flex-column" style={{ flex: 1, minWidth: 0, gap: '0.75rem' }}>
                    ...
                    <div className="d-flex w-100" style={{ flex: 1, minHeight: 0, gap: '0.75rem' }}>
                        {/* 1. Top 10 Customers */}
                        <div className="card shadow-sm border-0 p-2" style={{ flex: '0 0 calc(50% - 0.375rem)', maxWidth: 'calc(50% - 0.375rem)', minHeight: '220px', display: 'flex', flexDirection: 'column' }}>
                            <h6 className="fw-bold small mb-0 ms-2 text-start" style={{ color: '#34D399' }}>Top 10 Customer's job booked</h6>
                            <div style={{ width: '100%', height: 200, minHeight: 200, flexShrink: 0 }}>
                            <ResponsiveContainer width="100%" height={200} minHeight={200} debounce={50}>
