...
if (!fs.existsSync(logosDir)) {
    try {
        ...
        console.error('FAILED TO CREATE LOGOS DIR:', err.message);
    }
}
...
const app = express();
const PORT = 5002; // Hardcoded for production proxy alignment

// Middleware
app.use(cors());
app.use(express.json({ limit: '50mb' }));
app.use('/uploads', express.static(uploadDir)); // Serve from network share
...
app.post('/api/upload/logo', (req, res, next) => {
    ...
    // logosDir is already defined globally using the network share
    if (!fs.existsSync(logosDir)) {
        fs.mkdirSync(logosDir, { recursive: true });
    }
    next();
    ...
    try {
        ...
        // Move file to the logos subfolder on the network share
        const oldPath = req.file.path;
        const newPath = path.join(logosDir, req.file.filename);
